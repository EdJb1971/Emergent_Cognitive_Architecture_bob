...
| Operator-configured identity and clean start | Implemented | `IdentityService` owns a cached, local JSON profile outside cognitive storage. Bob is only the replaceable first-run default; assistant and optional operator names propagate through the browser title, conversation attribution, control room, proactive prompts, and Cognitive Brain. Configured identity explicitly outranks memory hints, and emotional memory no longer promotes arbitrary capitalized entities into a user name. Authenticated revision-checked APIs and the Settings workspace edit the profile. A separately confirmation-gated clean start is armed in the UI and consumed before databases open on the next restart, clearing the exact configured cognitive-data root while preserving identity by default. Both paths are git-ignored, so every clone starts without personal memory. |
...
| Predictive perception and calibration | Implemented in enforced shadow mode | `PredictivePerceptionService` forms bounded, explicitly labelled hypotheses from recent same-user STM context before inspecting the current immutable `SensoryEpisode`. Frozen errors and recommendations are durably copied into a separate SQLite WAL hash-chain ledger; database triggers reject edits and deletion. Authenticated same-user APIs and the Predictive operator workspace support list, inspect, independent append-only labels, superseding corrections, longitudinal calibration, cohort diagnostics, and ledger inspection. Missing evidence remains conservative. Contracts and calibration summaries hard-code no response, routing, research, learning, attention, or primary-evidence effects; metrics cannot self-activate influence. |
...
### Predictive Review and Calibration Plane

**Implementation:** `src/services/predictive_calibration_store.py`, the review contracts in `src/models/predictive_models.py`, `src/api/predictive_review.py`, `MetricsService`, startup lifecycle/health, and the React `PredictiveCalibrationCenter`.

1. Every waking assessment is copied after evaluation into a dedicated SQLite WAL ledger. `assessment_recorded` is idempotent per user and assessment; a unique index prevents duplicate originals. `UPDATE` and `DELETE` triggers enforce append-only storage and a global SHA-256 previous-hash chain detects mutation.
2. Assessment snapshots retain the complete frozen predictive contracts but no raw image or audio. Persistence failure is isolated and logged; it cannot fail or alter the waking response.
3. Authenticated APIs are same-user scoped: list and filter assessments, inspect one assessment with its latest labels, append a label, inspect longitudinal calibration, and page through the ledger. Unknown or cross-user targets return no record.
4. Human labels independently judge hypothesis correctness, observation quality, confirmed match/mismatch versus false conflict or missed mismatch, recommendation usefulness, preferred clarification/recapture action, outcome certainty, and rationale. A correction appends a superseding label; no assessment, hypothesis, error, or sensory episode is edited.
5. Calibration reports coverage, mismatch confusion/precision/recall, false-conflict rate, hypothesis accuracy, observation reliability, recommendation usefulness, preferred-action agreement, confidence-bin expected calibration error, 30-day cohorts, and strata by modality, prior source, reliability, feature/error status, and recommended action.
6. `predictive_influence_eligible` is a literal false value. No threshold or metric can change it. Predictive ledger telemetry contains IDs, counts, enum judgements, and invariant false flags only; semantic priors, observations, rationale, and sensory content are excluded.
7. The Predictive workspace separates Review, Calibration, and Ledger jobs. It presents prior and primary observation side by side, makes the immutable evidence boundary and shadow lock persistent, supports responsive append-only review, plots real daily coverage and confidence reliability, exposes diagnostic cohorts, and verifies the event hash chain.

**Validation boundary:** Deterministic tests cover no-prior behavior, source exclusion, hypothesis labelling and immutability, matched and signed mismatch cases, conservative unobserved and low-reliability states, unexecuted clarification/recapture, cross-modal clarification, disabled mode, non-shadow rejection, contract tamper rejection, malformed stored metadata, orchestration response preservation, database-enforced immutability, hash integrity, idempotency, label supersession, invalid/cross-user target rejection, calibrated confusion and ECE calculations, authenticated APIs, persistence failure isolation, and content-free typed telemetry. The frontend passes TypeScript and production compilation. No representative real-cycle prediction precision, calibration, false-conflict rate, or clarification usefulness has yet accumulated; therefore prediction errors cannot influence attention or learning.
