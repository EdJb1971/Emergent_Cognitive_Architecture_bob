...
| Validation | Repaired baseline | The repository virtual environment runs `228 passed, 3 skipped` on August 2, 2026. The suite includes visual/auditory validation and instruction boundaries plus immutable multisensory binding, temporal-window, agreement, explicit-negation, categorical-conflict, insufficient-evidence, reliability-weighting, advisory-only, raw-media-removal, cognition, memory, and telemetry assertions. Authenticated telemetry replay/gaps, post-commit projection, governed-work and sleep lifecycle/recovery, provenance, and a real ephemeral-Chroma semantic round trip remain green. Ollama declared `completion, vision, audio, tools, thinking`; live local visual and synthetic-tone auditory round trips validated transport, parsing, deterministic quality gates, and conservative non-speech behavior. The React production build passes TypeScript checking and Vite `8.2.0` compilation with Node `v24.18.1`; `npm audit` reports zero vulnerabilities. These are regression/connectivity results, not proof of perceptual accuracy, real-world calibration, or biological equivalence. The three skipped root-level async tests are not collected by an async test plugin. |
...
- The current full suite passes with `228 passed, 3 skipped`; salience-specific assertions remain green.
