...
- Multisensory temporal binding is implemented. Each turn creates one frozen `SensoryEpisode` over authoritative server-timed text plus typed visual/auditory evidence references. Deterministic comparison records conservative agreement, opposed polarity, shared-entity categorical conflicts, insufficient evidence, temporal exclusion, and reliability ceilings. Observable input/signal quality outweighs model confidence. Advisory attention reaches Perception and synthesis while routing, primary evidence, and raw-media boundaries remain unchanged.
- Predictive perception and active clarification are implemented in enforced shadow mode. Bounded hypotheses are formed from recent same-user user assertions and typed sensory context before current evidence is inspected, remain permanently labelled as non-observations, and produce immutable provenance-linked signed errors. Missing or low-quality evidence cannot become a material discrepancy. At most one clarification or image/audio recapture is recommended and never executed; response, routing, research, learning, and primary evidence remain untouched.

**Test baseline:** `252 passed, 3 skipped` on August 2, 2026. The backend suite includes visual/auditory and multisensory control boundaries plus same-user prior-only hypothesis formation, immutable signed prediction errors, assistant/previous-prediction/cross-user source exclusion, conservative missing/low-quality evidence, bounded unexecuted clarification/recapture, malformed-memory and evaluator-failure degradation, database-enforced append-only assessment/label history, hash integrity, label supersession, authenticated same-user APIs, longitudinal confusion/ECE calculations, unknown-outcome exclusion from calibration truth, predictive persistence failure isolation, content-free telemetry, and response/routing/research/learning non-influence. Authenticated telemetry replay/gaps, post-commit ledger projection, governed-work controls, governed-sleep lifecycle/recovery, provenance, and a real ephemeral-Chroma semantic round trip remain green. The frontend passes TypeScript checking and a Vite `8.2.0` production build with Node `v24.18.1`; `npm audit` reports zero findings. Live local `gemma4:e4b` visual and synthetic-tone auditory smokes validated capability, transport, JSON parsing, quality capping, and conservative non-speech behavior, but not real-world perceptual/cross-modal accuracy or predictive calibration. The skips are three root-level async tests not handled by an async test plugin. The 20-case synthetic research-drive fixture measures action accuracy `1.000`, escalation precision `1.000`, and escalation recall `1.000`; representative real-cycle predictive calibration observations have not yet accumulated.
...
**Status:** Predictive experiment plus durable human review/calibration plane implemented in enforced shadow mode; representative longitudinal evidence remains to be collected.

1. Complete: an interpretable prior-only perceptual baseline predicts bounded presence and categorical features from recent user/typed sensory context rather than using a learned world model.
2. Complete: frozen hypothesis, prediction-error, assessment, and unexecuted clarification/recapture contracts preserve provenance and make prediction/observation confusion structurally visible.
3. Complete: the experiment runs before routing and synthesis but is not supplied to either; it cannot invoke research, update learning, rewrite evidence, or alter the response.
4. Complete: every real-cycle assessment is durably copied into a database-enforced append-only hash chain; authenticated same-user APIs and a professional Predictive workspace support inspection and additive labels for hypotheses, observations, outcomes, and clarification/recapture recommendations.
5. Complete: longitudinal summaries report coverage, mismatch precision/recall, false-conflict rate, hypothesis accuracy, confidence-bin ECE, recommendation usefulness, daily cohorts, and diagnostic strata by modality, prior source, reliability, feature/error status, and recommended action. Eligibility remains a structural literal false rather than a threshold result.
6. Next measurement slice: build a held-out predictive evaluation and activation-readiness report with minimum representative sample/coverage gates, confidence intervals, drift detection, and adversarial false-conflict cohorts. It may report `blocked` or `ready_for_controlled_experiment`, but cannot enable runtime influence.
7. Later: only after separately reviewed evidence, evaluate whether a calibrated error signal improves advisory attention or context prefetching without increasing false conflict or irrelevant retrieval. Episodic future simulation and affective forecasting remain deferred until then.
...
19. Complete: multisensory temporal binding creates an immutable typed `SensoryEpisode` for every turn, aligns server-receipt text with relay-capture image/audio intervals, preserves source hashes/provenance/uncertainty, and conservatively distinguishes agreement, contradiction, insufficient evidence, and temporal exclusion. Reliability weights measured quality above model confidence. Derived attention is exposed to Perception and Cognitive Brain only; generative fusion, evidence rewriting, and routing changes are structurally false. Bounded memory, health, and content-free telemetry projections are wired.
20. Complete: predictive perception and active clarification run in enforced shadow. Frozen `prior_hypothesis_not_observation` records are formed before current observation extraction from at most three same-user STM cycles and eight reviewable features. Immutable signed errors reference primary `SensoryEpisode` evidence; missing evidence remains unobserved and low-reliability evidence cannot be material. One bounded clarification or sensory-recapture recommendation may be recorded but cannot execute. Assistant output and previous predictions are excluded as priors; malformed memory or evaluator failure degrades to an explicit empty typed assessment without failing the waking cycle; response, routing, research, learning, and evidence-rewrite flags are structurally false. Health, bounded autobiographical summary, and content-free live telemetry are wired.
21. Complete: the predictive review and calibration plane durably persists immutable assessments and additive/superseding judgements in a SQLite WAL ledger protected by update/delete triggers, per-assessment uniqueness, and a verified SHA-256 hash chain. Authenticated same-user list/inspect/label/calibration/ledger APIs are wired through startup and health. The responsive Predictive workspace provides prior-versus-observation review, independent hypothesis/observation/outcome/action labels, daily and confidence calibration visualizations, cohort diagnostics, and ledger inspection. Post-commit typed telemetry is content-free. Persistence failure cannot fail a waking cycle, and `predictive_influence_eligible` is structurally false.
22. Complete: add an authoritative local identity and clean-start plane. Bob remains only the replaceable first-run default; an optional operator name defaults blank. Revision-checked authenticated APIs, cached hot-path identity context, and a responsive Settings workspace propagate names through the title, brand, chat labels, control room, Cognitive Brain, and proactive generation. Capitalized summary entities can no longer become the user's name. Git-ignored identity survives a separately typed-confirmation, cancellable, restart-boundary cognitive-memory reset; fresh clones contain neither identity nor memory state.
23. **Next measurement slice:** add a reproducible held-out predictive evaluation and activation-readiness report. Require representative labelled sample and coverage floors across modality/source/reliability/error/action strata, confidence intervals for mismatch precision/recall and false-conflict rate, calibration drift checks, and adversarial conflict fixtures. The report can recommend whether a separately designed controlled experiment is justified; it must not toggle attention, retrieval, learning, or response influence.

The next implementation slice remains measurement, not activation. The system now persists reviewable real-cycle artifacts and independent human judgements, but deterministic fixtures and empty/early calibration tables do not establish that its priors are calibrated or its clarifications useful. Define representative sample gates and uncertainty-aware readiness reporting while labels accumulate. In parallel, collect human-labelled image, clean speech, silence, tone, noise, and adversarial cross-modal fixtures; successful transport and deterministic binding tests are not perceptual-accuracy benchmarks.
