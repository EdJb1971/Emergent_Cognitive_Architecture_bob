...
**Test baseline:** `228 passed, 3 skipped` on August 2, 2026. The backend suite includes visual and auditory validation/capability/locality/instruction-boundary tests plus frozen multisensory episodes, temporal alignment/exclusion, conservative agreement/conflict/insufficient-evidence classification, reliability-aware advisory attention, raw-media removal, synthesis boundaries, authenticated telemetry replay/gaps, post-commit ledger projection, governed-work controls, governed-sleep lifecycle/recovery, provenance, and a real ephemeral-Chroma semantic round trip. The frontend passes TypeScript checking and a Vite `8.2.0` production build with Node `v24.18.1`; `npm audit` reports zero findings. Live local `gemma4:e4b` visual and synthetic-tone auditory smokes validated capability, transport, JSON parsing, quality capping, and conservative non-speech behavior, but not real-world perceptual or cross-modal accuracy. The skips are three root-level async tests not handled by an async test plugin. The 20-case synthetic research-drive fixture measures action accuracy `1.000`, escalation precision `1.000`, and escalation recall `1.000`; representative real-cycle calibration observations have not yet accumulated.
...
20. **Next code slice:** implement predictive perception and active clarification in shadow: a typed prior/hypothesis contract predicts only reviewable features from recent trusted context, compares those predictions with immutable `SensoryEpisode` evidence, emits signed prediction errors with provenance and calibration fields, and recommends a bounded user clarification or sensory re-capture when a material high-value conflict remains unresolved. It must never fill missing evidence, promote an LLM guess into observation, invoke cloud research, or autonomously change routing.

The next implementation slice is the cortical predictive-processing loop. The key safety distinction is between a prediction and an observation: priors remain labelled hypotheses, sensory evidence remains primary, and only the discrepancy is new derived data. Start in shadow so precision, false-conflict rate, clarification usefulness, and sensory-model calibration can be measured before any prediction-error signal influences attention or learning. In parallel, collect human-labelled image, clean speech, silence, tone, noise, and adversarial cross-modal fixtures; successful transport and deterministic binding tests are not perceptual-accuracy benchmarks.
