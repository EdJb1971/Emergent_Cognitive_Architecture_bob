...
**Test baseline:** `154 passed, 3 skipped` on August 2, 2026. The skips are three root-level async tests that are not handled by an async test plugin. The 20-case synthetic research-drive fixture measures action accuracy `1.000`, escalation precision `1.000`, and escalation recall `1.000`; the real-cycle capture/label/summary mechanism is implemented but representative labeled observations have not yet accumulated.
...
**Current status:** The provider-neutral safety boundary, cognitive research drive, persistent inquiry queue, waking revalidation, grounded Gemini provider, strict packet validation, citation-aware local synthesis, authenticated review APIs, and append-only calibration ledger are implemented. Legacy `WebBrowsingService` is unreachable. The complete path is tested with fakes and one guarded live grounding smoke. Runtime defaults remain disabled/shadowed; representative real-cycle labeling, a React operator view, and broader operational/source-quality evaluation remain pending.
...
11. Complete: add the waking inquiry review and calibration API: authenticated list/inspect/approve/dismiss/retry operations, a database-enforced append-only hash chain, persisted decisions/packets, verified-source feedback, real-cycle shadow observations and labels, paginated history, and calibration summaries. Approval remains consent rather than an authorization bypass, and the summary hard-reports automatic non-explicit research as ineligible.
12. **Next:** operate the shadow-calibration study and add the React operator view. Accumulate representative real-cycle observations, review false positives/negatives across stakes/privacy/volatility strata, collect post-answer and source-quality outcomes, expose queue/label/precision/recall views in the dashboard, and pre-register minimum evidence and rollback criteria. Do not change activation defaults in this slice.
13. Then continue Phase 6 observability and Phase 8 validation, followed by attention, salience, and autonomous-task evaluation before predictive cognition.

The next slice is evidence collection and operator usability, not automatic research. The backend can now capture everything required, but a calibration mechanism without representative labels is only instrumentation. Build the operator view over the existing APIs, run ordinary cycles in shadow, label recommendations independently of the controller, and stratify results by explicit request, stakes, volatility, privacy inhibition, source quality, and whether external evidence changed the answer. Define a minimum sample, acceptable false-positive ceiling, required recall for high-stakes volatile facts, and a rollback plan before a separate activation decision is even considered.
