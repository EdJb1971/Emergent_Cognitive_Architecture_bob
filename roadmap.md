...
**Test baseline:** `242 passed, 3 skipped` on August 2, 2026. The backend suite includes visual/auditory and multisensory control boundaries plus same-user prior-only hypothesis formation, immutable signed prediction errors, assistant/previous-prediction/cross-user source exclusion, conservative missing/low-quality evidence, bounded unexecuted clarification/recapture, malformed-memory and evaluator-failure degradation, content-free telemetry, and response/routing/research/learning non-influence. Authenticated telemetry replay/gaps, post-commit ledger projection, governed-work controls, governed-sleep lifecycle/recovery, provenance, and a real ephemeral-Chroma semantic round trip remain green. The frontend passes TypeScript checking and a Vite `8.2.0` production build with Node `v24.18.1`; `npm audit` reports zero findings. Live local `gemma4:e4b` visual and synthetic-tone auditory smokes validated capability, transport, JSON parsing, quality capping, and conservative non-speech behavior, but not real-world perceptual/cross-modal accuracy or predictive calibration. The skips are three root-level async tests not handled by an async test plugin. The 20-case synthetic research-drive fixture measures action accuracy `1.000`, escalation precision `1.000`, and escalation recall `1.000`; representative real-cycle calibration observations have not yet accumulated.
...
**Status:** First interpretable experiment implemented in enforced shadow mode; calibration work remains planned.
...
4. Next: add durable operator labels and longitudinal calibration by modality, source kind, reliability, error class, and recommended action before considering any influence.
5. Later: evaluate whether a calibrated error signal improves advisory attention or context prefetching without increasing false conflict or irrelevant retrieval. Episodic future simulation and affective forecasting remain deferred until then.
...
21. **Next code slice:** add the predictive review and calibration plane before any activation: a durable append-only assessment/decision ledger, authenticated list/inspect/label APIs, and an operator UI for marking hypothesis correctness, observation quality, real versus false mismatch, and clarification/recapture usefulness. Report longitudinal calibration and false-conflict rates by modality, prior source, reliability stratum, feature/error class, and recommended action. Labels must never rewrite the original hypothesis, error, or sensory episode, and metrics must never self-enable attention or learning influence.

The next implementation slice is measurement, not activation. The system now creates reviewable shadow artifacts, but deterministic fixtures do not establish that its priors are calibrated or its clarifications useful. Persist immutable real-cycle assessments and append-only human judgements so precision, calibration error, false-conflict rate, and action value can be measured before any prediction-error signal is allowed near attention, retrieval, or learning. In parallel, collect human-labelled image, clean speech, silence, tone, noise, and adversarial cross-modal fixtures; successful transport and deterministic binding tests are not perceptual-accuracy benchmarks.
