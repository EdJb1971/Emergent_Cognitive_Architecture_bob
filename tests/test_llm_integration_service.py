...
async def test_llm_service_init_no_api_key():
    with patch('src.services.llm_integration_service.settings.GEMINI_API_KEY', ""):
...
async def test_generate_text_success(mock_generative_model, llm_service):
    ...
    mock_generative_model.return_value.generate_content_async = AsyncMock(return_value=mock_response)
...
async def test_generate_text_blocked_prompt(mock_generative_model, llm_service):
    ...
    mock_response.prompt_feedback.block_reason = MagicMock(name="SAFETY")
...
async def test_generate_text_api_error(mock_generative_model, llm_service):
    mock_generative_model.return_value.generate_content_async = AsyncMock(side_effect=Exception("API error"))
...
async def test_moderate_content_unsafe(mock_generative_model, llm_service):
    mock_response = MagicMock()
    mock_response.prompt_feedback = AsyncMock()
    mock_response.prompt_feedback.block_reason = MagicMock(name="HARASSMENT")
    mock_response.prompt_feedback.block_reason.name = "HARASSMENT"
...
async def test_moderate_content_blocked_prompt_exception(mock_generative_model, llm_service):
    mock_generative_model.return_value.generate_content_async = AsyncMock(side_effect=BlockedPromptException("Blocked by LLM"))
...
async def test_moderate_content_api_error(mock_generative_model, llm_service):
    mock_generative_model.return_value.generate_content_async = AsyncMock(side_effect=Exception("Moderation API error"))
...
async def test_generate_text_empty_prompt(llm_service):
    with pytest.raises(LLMServiceException, match="At least one of prompt"):
        await llm_service.generate_text("")

@pytest.mark.asyncio
@patch('google.generativeai.GenerativeModel')
async def test_generate_text_retry_mechanism(mock_generative_model, llm_service):
    mock_generative_model.return_value.generate_content_async = AsyncMock(side_effect=[
        LLMServiceException(detail="Transient error"),
        LLMServiceException(detail="Another transient error"),
        MagicMock(candidates=[MagicMock(content=MagicMock(parts=[MagicMock(text="Success after retry")]))]),
    ])
...
async def test_generate_text_retry_failure(mock_generative_model, llm_service):
    mock_generative_model.return_value.generate_content_async = AsyncMock(
        side_effect=LLMServiceException(detail="Persistent error")
    )
